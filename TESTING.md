...
## جولة: إضافة JSearch كمصدر ثانٍ مدموج مع Jooble (لا استبدال)

### الطلب
بما أن الجولة السابقة أثبتت أن ندرة بيانات Jooble لمسميات/دول معينة
(مثل "Backend Developer" في السعودية) سبب خارجي غير قابل للإصلاح
برمجياً، طُلب إضافة **مصدر ثانٍ** (JSearch عبر RapidAPI) يُدمَج مع
Jooble لا يستبدله — بترشيح تكرار ذكي (تفضيل النسخة الأدق جغرافياً)،
سقف نتائج معقول، وتحديث منطق ملاحظة "نتائج بمستوى الدولة فقط" لتظهر فقط
لو **كلا** المصدرين فشلا بمطابقة المدينة، لا Jooble وحده.

### حالة اختبار 104 — بناء tools/jsearch_client.py بنفس نمط الصمود الموجود أصلاً
عميل جديد مستقل (`search_jsearch(query, location)`) بنفس فلسفة الصمود
المعتمدة أصلاً بـ`job_search.py`: يرجع **قائمة فارغة** (لا استثناء) عند
أي فشل — مفتاح ناقص، شبكة، JSON غير مقروء، أو شكل رد غير متوقَّع — مع
تسجيل السبب بـ`print`. يبني نص بحث JSearch بصيغة `"{query} in {location}"`
كما يتوقعها API نفسها، ويحوّل كل عنصر رد لنفس الشكل الداخلي المُستخدَم
لـJooble (`title`, `company`, `location`, `link`, `snippet`) + حقل
`source: "JSearch"` إضافي، فلا يحتاج أي كود لاحق معرفة مصدر النتيجة.

### حالة اختبار 105 — الدمج الفعلي في job_search.py: Jooble + JSearch
`search_jobs()` أُعيدت هيكلتها: استُخرج استدعاء Jooble لدالة داخلية
`_search_jooble()` بنفس نمط الصمود (قائمة فارغة عند الفشل، لا استثناء)
بدل `{"error": ...}` القديمة التي كانت تُسقط الأداة بالكامل لو المفتاح
ناقص — الآن فشل مصدر واحد لا يمنع الآخر من إرجاع نتائج، تماشياً مع
قاعدة CLAUDE.md رقم 6. دالة جديدة `_dedupe_merge()` تدمج القائمتين
بمفتاح `(title.lower(), company.lower())`: عند التكرار، تُفضَّل نسخة
JSearch لو موقعها الفعلي يحوي اسم المدينة المطلوبة حرفياً (أدق جغرافياً
من تصنيف Jooble القُطري كما وُثِّق بالجولة السابقة)، وإلا تبقى أول نسخة
(Jooble، لأنه المصدر الأول بترتيب الدمج). النتائج مقصوصة لأعلى 10
(`MAX_COMBINED_RESULTS`). منطق ملاحظة "نتائج بمستوى الدولة" أُعيد ربطه
بالنتائج **المدموجة** لا Jooble وحدها — يظهر فقط لو لا نتيجة من أي
مصدر تحوي اسم المدينة.

### حالة اختبار 106 — اختبار حي حقيقي: JSEARCH_API_KEY الموجود بـ.env غير مُفعَّل فعلياً (403)
استدعاء JSearch API مباشرة (بمعزل عن كودنا، بايثون + requests صرف)
بنفس المفتاح الموجود بـ.env كشف: `403 {"message":"You are not
subscribed to this API."}` — المفتاح صالح شكلياً (RapidAPI) لكن غير
مُفعَّل فعلياً لمنتج JSearch تحديداً (كل API على RapidAPI يحتاج اشتراكاً
منفصلاً حتى بنفس المفتاح) — قيد حساب خارجي حقيقي، لا علة بكودنا (نفس
فئة اكتشاف الجولة السابقة مع Jooble، مؤكَّد بدليل مباشر لا افتراض).
هذه الحالة **بالضبط** ما طلبه المستخدم اختباره ("JSEARCH_API_KEY غير
صالح مؤقتاً") — لم تكن هناك حاجة لإبطال المفتاح يدوياً، فهو غير مُفعَّل
فعلياً لهذا المنتج أصلاً.

**القياس عبر `search_jobs()` الفعلية (لا HTTP، استدعاء مباشر) بالمدن
الثلاث بنفس المفتاح غير المُفعَّل**:
```
Jeddah, Saudi Arabia  -> total_count=4  note=موجودة  (Jooble فقط، JSearch أرجعت فارغة بصمت)
Riyadh, Saudi Arabia  -> total_count=4  note=موجودة  (نفس السلوك)
Makkah, Saudi Arabia  -> total_count=4  note=موجودة  (نفس السلوك)
```
كل النتائج الأربع بكل مدينة موسومة `source: "Jooble"` بشكل صحيح —
**صفر استثناء، صفر انهيار** للتدفق الموحَّد رغم فشل أحد المصدرين بالكامل
بكل استدعاء. تحقّق أيضاً عبر الواجهة الفعلية (Playwright، رفع PDF حقيقي
+ بحث حقيقي): بطاقات الوظائف تُرسَم بشكل طبيعي تماماً، **صفر أخطاء
console** (لقطة `MERGED_FALLBACK_UI.png`).

### حالة اختبار 107 — إثبات صحة منطق الدمج/الترشيح بمعزل عن حالة الاشتراك (بيانات JSearch محاكاة واقعية الشكل)
بما أن المفتاح الحقيقي غير مُفعَّل لمنتج JSearch (خارج تحكّمنا)، تعذّر
جمع دليل حي لتباين نتائج JSearch الفعلية حسب المدينة. للتحقق من **صحة
كود الدمج نفسه** (لا حالة الحساب الخارجي)، استُبدلت `search_jsearch`
داخل `tools.job_search` فقط (monkeypatch على مستوى الاختبار، لا تعديل
لأي سطر إنتاجي) بردود واقعية الشكل (نفس حقول JSearch الحقيقية) تختلف
فعلياً حسب المدينة، عبر استدعاء `search_jobs()` الحقيقية غير المُعدَّلة:
```
Jeddah  -> 6 نتائج (4 Jooble + 2 JSearch حقيقيتا الشكل: "STC Solutions"، "Elm Company")   note=غائبة (JSearch غطّت المدينة)
Riyadh  -> 5 نتائج (4 Jooble + 1 JSearch: "stc pay")                                       note=غائبة
Makkah  -> 4 نتائج (Jooble فقط — JSearch الوهمية أرجعت فارغة لهذي المدينة تحديداً)          note=موجودة (كلا المصدرين فشلا هنا)
```
**اختبار ترشيح التكرار تحديداً**: أُرجعت وظيفة "Staff Backend Engineer
(NodeJS/Go)" في شركة Yassir من **كلا** المصدرين (نفس العنوان/الشركة
حرفياً)، JSearch الوهمية بموقع "Jeddah, Saudi Arabia" (أدق من موقع
Jooble "Saudi Arabia" وحدها):
```
عدد النسخ بالنتيجة النهائية: 1  (متوقَّع 1 لا 2 — الدمج أزال التكرار) ✅
المصدر المُبقى عليه: JSearch     (متوقَّع JSearch — موقعه يحوي اسم المدينة) ✅
موقعه الفعلي: Jeddah, Saudi Arabia
note (متوقَّع غائبة): غائبة ✅
```
يثبت هذا أن منطق `_dedupe_merge()` وملاحظة "مستوى الدولة" يعملان بدقة
تامة **متى ما توفّرت بيانات JSearch فعلية** — العائق الوحيد لرؤية هذا
حياً بالواجهة هو اشتراك حساب RapidAPI، لا الكود.

### التحقق النهائي
- **صفر تغيير على منطق Jooble نفسه أو تحويل المدينة بـ`agent.py`** —
  الدمج إضافة صرفة، لا استبدال. ✅
- **صمود حقيقي مؤكَّد بدليل حي**: فشل JSearch الكامل (403 حقيقي غير
  مُصطنَع) لم يُسقط البحث، ولا حتى استثناءً واحداً — تحقَّق عبر الاستدعاء
  المباشر والواجهة معاً. ✅
- **صفر أخطاء console** بالتحقق عبر الواجهة الحقيقية. ✅
- `.env.example` يحوي الآن `JSEARCH_API_KEY=` (بلا قيمة حقيقية، بنفس
  نمط المفتاحين الآخرين). ✅ `requirements.txt` لم يحتج أي إضافة —
  `requests` موجودة أصلاً وتُستخدَم لكلا المصدرين. ✅
- حقل `source` موجود بكل نتيجة (`"Jooble"` أو `"JSearch"`) لكن **غير
  معروض بارز بالواجهة** كما طُلب صراحة — موثَّق هنا فقط كدليل.

### الخلاصة
JSearch أُضيف كمصدر ثانٍ مدموج فعلياً مع Jooble، لا بديلاً عنه، بنفس
فلسفة الصمود المعتمدة بالمشروع (فشل مصدر لا يُسقط الآخر ولا الأداة
كاملة). التحقق الحي كشف قيداً خارجياً حقيقياً آخر (مفتاح JSearch
الحالي غير مُشترَك فعلياً بمنتج JSearch على RapidAPI، `403`) — وثّق هذا
بدليل مباشر بدل الافتراض، وأثبت أن التدفق الموحَّد يتحمّله بصمود كامل
(Jooble فقط، بلا انهيار). بما أن هذا القيد خارج تحكّم الكود، صحة منطق
الدمج/الترشيح/الملاحظة نفسه أُثبتت بمعزل عنه عبر بيانات محاكاة واقعية
الشكل تستبدل فقط دالة الشبكة الخارجية (`search_jsearch`) بلا لمس لأي
سطر إنتاجي آخر — أظهرت تبايناً حقيقياً حسب المدينة، وترشيح تكرار صحيحاً
يفضّل الأدق جغرافياً، وملاحظة تظهر/تختفي بدقة حسب مطابقة كلا المصدرين
معاً كما طُلب بالضبط.

## جولة: تسريب مفتاح RapidAPI + استبدال JSearch بـIndeed Scraper API وActive Jobs DB

### الطلب
تنبيه أمني: مفتاح RapidAPI السابق (`29a4f0663...`) تسرّب بلصقه نصاً صريحاً
بالمحادثة، ويُعتبَر مخترَقاً. طُلب: (1) فحص شجرة العمل وتاريخ git بالكامل
عن أثر المفتاح القديم قبل أي شيء آخر، (2) إسقاط تكامل JSearch كاملاً
(نقطة `/job-details` كانت خاطئة أصلاً — تحتاج `job_id` موجوداً مسبقاً، لا
تصلح للبحث)، واستبداله بمصدرين: **Indeed Scraper API** (أساسي) و**Active
Jobs DB** (ثانوي، يُستخدَم فقط لو جودة رده جيدة)، بنفس نمط الصمود والدمج
المعتمد، واختبار المدن الثلاث (جدة، الرياض، مكة) بالمفتاح **الجديد فقط**.

### حالة اختبار 108 — الفحص الأمني: هل تسرّب المفتاح القديم لـgit فعلياً؟
```
grep -rn "29a4f0663" (شجرة العمل بالكامل)          -> وجود وحيد في .env فقط (مُستثنى من git أصلاً بـ.gitignore)
git log --all -S"29a4f0663" (بحث pickaxe بكل التاريخ/كل المراجع) -> صفر نتائج
git rev-list --all | xargs git grep "29a4f0663"    -> صفر نتائج بأي commit عبر كامل التاريخ
```
**النتيجة**: المفتاح القديم **لم يُلتزَم به إطلاقاً بأي commit أو أي فرع
طوال تاريخ المستودع** — كان موجوداً فقط بملف `.env` المحلي غير المتتبَّع.
لا حاجة لتنظيف تاريخ git (لا `filter-branch`/`BFG`). المفتاح القديم أُزيل
من `.env` فوراً (استُبدل بـ`RAPIDAPI_KEY=` فاضية) بدل إبقائه حتى لو غير
مُستخدَم بالكود بعد الآن — تقليل مساحة أي تسريب لاحق عرضي.

### حالة اختبار 109 — إسقاط JSearch، بناء tools/rapidapi_client.py (Indeed Scraper + Active Jobs DB)
حُذف `tools/jsearch_client.py` بالكامل (`git rm`)، وأُنشئ
`tools/rapidapi_client.py` بدالتين: `search_indeed_scraper()` (أساسي،
POST لـ`indeed-scraper-api.p.rapidapi.com/api/job` بالبايلود المطلوب
حرفياً: `maxRows/query/location/country="sa"/sort="relevance"/
fromDays="14"`) و`search_active_jobs_db()` (ثانوي، GET لـ`active-jobs-
db.p.rapidapi.com/active-ats`). كلاهما بنفس فلسفة الصمود: قائمة فارغة +
تسجيل السبب عند أي فشل (مفتاح ناقص، شبكة، JSON غير مقروء، شكل رد غير
متوقَّع) — لا استثناء أبداً. المفتاح يُقرأ حصراً من `RAPIDAPI_KEY` بـ.env،
لا يُطبَع كاملاً بأي رسالة تسجيل (كل رسائل الخطأ تطبع سبب الفشل فقط، لا
الترويسات ولا المفتاح).

**"جودة الرد" لـActive Jobs DB (الشرط الثانوي المطلوب)**: عُرِّفت عملياً
بأنها "رد ناجح ببنية مفهومة يحوي عنصراً واحداً على الأقل" — أي رد فاشل/
فارغ/مشوَّه يُقصى ذاتياً عبر آلية الصمود نفسها، فلا حاجة لفحص جودة منفصل
إضافي؛ قرار الدمج الفعلي (تُدمَج أم لا) يُحسَم داخل `_dedupe_merge()` بلا
تمييز مصدر ثانوي/أساسي صريح — أي نتيجة صالحة من أي مصدر تُدمَج.

### حالة اختبار 110 — علل تكامل حقيقية اكتُشفت بالاختبار الحي (لا بالتوثيق النظري) بالمفتاح الجديد
مواصفة المهمة لم تطابق تماماً الشكل الفعلي لأي من الـAPIين — كلتا العلتين
اكتُشفتا بفحص الرد الخام فعلياً (محفوظ بملفات مؤقتة، لا مطبوعاً بالكونسول
مباشرة، تجنباً لأي احتمال طباعة المفتاح بالخطأ):

**Indeed Scraper API** — منتج يعمل كطابور مهام (Bull queue)، لا استجابة
بحث مباشرة:
- الرد الأعلى هو حالة المهمة نفسها (`state`, `id`, `progress`, ...)، لا
  قائمة وظائف — النتائج الفعلية متداخلة بمستوى إضافي: `returnvalue.data`
  (قائمة)، لا `returnvalue` مباشرةً كما افترض الكود الأول.
- `location` بكل عنصر وظيفة **كائن متداخل** (`{city, country,
  formattedAddressShort, ...}`) لا نصاً مباشراً — كان سيُسبب خطأً حقيقياً
  (`AttributeError` عند `.lower()`) لو مُرِّر كما هو لمنطق مطابقة المدينة
  بـ`job_search.py`. أُصلح باستخراج `formattedAddressShort` تحديداً (نص
  إنجليزي مطابق لصيغة تطبيقنا، بعكس حقل `city` الذي رجع بالعربية أحياناً
  "جدة" لا "Jeddah").
- **لا حقل اسم شركة إطلاقاً** بأي من العناصر المُختبَرة فعلياً (تحقّق عبر
  8 نتائج مختلفة: `companyCeo.name`/`companyAddresses` فاضية بكل واحدة) —
  حقل `company` يبقى فاضياً بصمت بدل اختلاق اسم (يطابق مبدأ عدم الاختلاق
  المعتمد بالمشروع لأدوات أخرى).
- الوصف الفعلي بحقل `descriptionText` (نص صرف) لا `description` كما
  افتُرض أولاً.

**Active Jobs DB API**:
- استدعاء أول بلا `time_frame` أرجع **400 صريح**: `"Missing or invalid
  'time_frame' parameter. Must be one of: 1h, 24h, 7d, 6m"` — حقل إلزامي
  لم يُذكَر بمواصفة المهمة الأصلية إطلاقاً. أُضيف `time_frame="7d"` (نافذة
  متوازنة من القيم الأربع المسموحة).
- `location` الفعلي بحقل `locations_derived` (قائمة نصوص جاهزة التنسيق
  الكامل مثل `"Riyadh, Riyadh Region, Saudi Arabia"`) لا `locations_raw`
  كما افتُرض أولاً — أدق بكثير من `cities_derived` (اسم مدينة مجرَّد).
- لا حقل وصف/snippet إطلاقاً بهذا المنتج (بيانات وصفية بحتة) — يبقى
  فاضياً بصمت، مطابق لنفس المبدأ أعلاه.

كل هذي العلل أُصلحت بالكود قبل أي التزام نهائي — لم يُترَك أي منها.

### حالة اختبار 111 — الاختبار الحي الكامل: 3 مدن، دليل تباين فعلي حسب المدينة
استدعاء مباشر لكل مصدر على حدة (بلا HTTP، بايثون صرف) بنفس المفتاح
الجديد، 3 مدن، نفس المسمى "Backend Developer":
```
Jeddah, Saudi Arabia:  Jooble=4   Indeed=8    ActiveJobsDB=0
Riyadh, Saudi Arabia:  Jooble=4   Indeed=10   ActiveJobsDB=1
Makkah, Saudi Arabia:  Jooble=4   Indeed=10   ActiveJobsDB=0
```
**دليل التباين الفعلي حسب المدينة (عناوين ومواقع حقيقية من Indeed
Scraper، لا محاكاة)**:
```
جدة:    Web Solutions Developer | Biotech Software Developer | Municipal Software Developer ...  (location: "Jeddah")
الرياض: Back End Developer @ Datamatics Global Services | Frontend Developer @ Datamatics ...     (location: "Riyadh")
مكة:    Food Industry Software Developer | Software Engineer (Frontend/Backend) ...                (location: "Jeddah" أو "Mecca")
```
عناوين ونتائج **مختلفة فعلياً بالكامل** بين جدة والرياض (لا تكرار أي
وظيفة واحدة بينهما) — يثبت أن التصنيف الجغرافي لدى Indeed Scraper API
أدق بكثير من مستوى الدولة الذي كانت تقتصر عليه Jooble وحدها (الجولة
السابقة).

**عبر `search_jobs()` المدموجة الفعلية (لا الدوال منفردة)**:
```
Jeddah  -> total_count=13  note=غائبة  (Indeed غطّى المدينة بدقة)
Riyadh  -> total_count=13+ note=غائبة  (Indeed + ActiveJobsDB غطّيا المدينة)
Makkah  -> total_count=14  note=موجودة (انظر أدناه — سبب حقيقي مختلف عن الجولة السابقة)
```
**ملاحظة صادقة عن حالة مكة تحديداً**: النتيجة تحوي فعلياً وظائف بموقع
"Mecca" (النسخة الإنجليزية الشائعة لاسم المدينة لدى Indeed)، لكن الملاحظة
ظهرت رغم ذلك لأن منطق المطابقة الحالي (`_dedupe_merge`/فحص الملاحظة) يقارن
نصياً بـ"makkah" (الجزء الأول من `location` كما يُطبِّعه Gemini)، و"makkah"
ليست جزءاً نصياً من "mecca" — **تفاوت تهجئة حقيقي بين مصدرين مختلفين
(Makkah/Mecca)**، لا عطلاً بمنطق الدمج نفسه (الذي أثبت صحته بجولتين
متتاليتين الآن). موثَّق صراحةً كقيد معروف، لا مُصلَح بهذي الجولة (خارج
نطاق الطلب المحدد — "أصلح فقط ما طُلب").

### حالة اختبار 112 — التحقق النهائي عبر الواجهة الحقيقية (رفع PDF + بحث حي بالمفتاح الجديد)
Playwright: رفع `data/demo_resume.pdf` فعلياً، بحث حقيقي "Backend
Developer" في "Riyadh" (٢٥ ثانية انتظار — 3 استدعاءات API خارجية متتالية
+ Gemini نفسه، أبطأ ملحوظاً من مصدر Jooble وحده سابقاً، متوقَّع مع 3
مصادر حقيقية بدل مصدر واحد). **10 بطاقات وظائف مرسومة فعلياً** (السقف
`MAX_COMBINED_RESULTS`)، أسماء شركات حقيقية ("Datamatics Global Services
Ltd") ظاهرة بشكل صحيح، بطاقات بلا اسم شركة (Indeed الفاضية) تُعرَض بشارة
شركة فاضية بلا كسر تخطيطي. **صفر أخطاء console** (لقطة
`MERGED_SOURCES_RIYADH.png`).

### التحقق النهائي
- **صفر أثر للمفتاح القديم بتاريخ git** — مؤكَّد بحثاً شاملاً، لا حاجة
  لتنظيف تاريخ. ✅
- **المفتاح الجديد لم يُطبَع/يُسجَّل كاملاً بأي مكان** طوال هذي الجولة —
  كل الفحص المباشر للردود الخام حُفظ بملفات مؤقتة خارج المستودع، لا
  بمخرجات console. ✅
- **تدفق مدموج حقيقي من 3 مصادر يعمل حياً**: Jooble + Indeed Scraper +
  Active Jobs DB، بأدلة تباين فعلية حسب المدينة (لا نتائج مكرَّرة بين
  المدن كما كانت العلة الأصلية مع Jooble وحدها). ✅
- **صفر تغيير على منطق Jooble أو تحويل المدينة بـ`agent.py`**. ✅
- `.env.example` يحوي الآن `RAPIDAPI_KEY=` (بدل `JSEARCH_API_KEY=`
  القديم). ✅

### الخلاصة
الفحص الأمني أثبت بدليل شامل (لا افتراض) أن المفتاح المُسرَّب لم يصل
لتاريخ git إطلاقاً — تسريب حقيقي بالمحادثة لكن بلا أثر دائم بالمستودع.
استبدال JSearch بـIndeed Scraper API + Active Jobs DB كشف بالاختبار الحي
(لا التوثيق النظري) عدة علل تكامل حقيقية غير متوقَّعة من مواصفة المهمة
الأصلية (شكل رد طابور مهام متداخل، حقل موقع كائن لا نص، حقل زمني إلزامي
غير مذكور، أسماء حقول فعلية مختلفة عمّا افتُرض) — أُصلحت كلها قبل أي
التزام، بأدلة رد خام محفوظة محلياً (لا بالمستودع، لا بمخرجات مطبوعة).
النتيجة النهائية: بحث بمدن مختلفة يرجع فعلياً نتائج مختلفة (عناوين
وشركات ومواقع حقيقية متمايزة)، محقِّقاً الهدف الأصلي من كامل هذي السلسلة
من الجولات — تحقّق حي كامل عبر الواجهة الفعلية بالمفتاح الجديد فقط.

## جولة: إصلاح توحيد تهجئات المدن (Makkah/Mecca) قبل مقارنة المطابقة

### الطلب
علة موثَّقة صراحةً بالجولة السابقة كقيد معروف غير مُصلَح: ملاحظة "نتائج
بمستوى الدولة فقط" كانت تظهر خطأً لمدينة مكة رغم وجود نتائج Indeed فعلية
بموقع "Mecca" مطابقة تماماً — لأن المقارنة نصية صرفة (`"makkah" in
"mecca"` = خطأ، رغم كونهما نفس المدينة). طُلب إضافة جدول توحيد تهجئات
(Makkah/Mecca، Madinah/Medina، Jeddah/Jiddah على الأقل) يُطبَّق على طلب
المستخدم وموقع كل نتيجة معاً قبل أي مقارنة.

### حالة اختبار 113 — بناء _normalize_city() وجدول CITY_ALIASES
أُضيف بـ`tools/job_search.py`: قاموس `CITY_ALIASES` يربط تهجئات بديلة
شائعة بصيغة موحَّدة (canonical) واحدة — `mecca→makkah`،
`medina/al-madinah/al madinah→madinah`، `jiddah→jeddah` (الثلاثة
المطلوبة صراحةً بالحد الأدنى؛ لم تُكتشَف أي تهجئة بديلة إضافية بحاجة
تغطية أثناء الاختبار الحي لهذي الجولة، فلم تُضَف مدن أخرى تخميناً بلا
دليل). دالة `_normalize_city(text)` تُحوِّل النص لحروف صغيرة ثم تستبدل
كل صيغة بديلة موجودة بصيغتها الموحَّدة. طُبِّقت بثلاث نقاط استخدام: (1)
حساب `requested_city` بـ`search_jobs()` (تُطبَّع فور اشتقاقها من
`location`)، (2) داخل `_dedupe_merge()` عند مقارنة موقع كل نتيجة (سواء
الأصلية أو الموجودة `existing`)، (3) فحص `city_matched` قبل بناء `note`.
لا حفظ للنسخة المُطبَّعة داخل بيانات النتيجة نفسها — `location` المعروضة
للمستخدم تبقى كما رجعت من المصدر الأصلي (مثلاً "Mecca")، التطبيع لغرض
المقارنة الداخلية فقط.

**اختبار وحدة مباشر (9 حالات، صفر مطابقة يدوية بالعين)**:
```
normalize('makkah')                      = 'makkah'   ✅
normalize('Mecca')                       = 'makkah'   ✅
normalize('Mecca, Saudi Arabia')         = 'makkah, saudi arabia' ✅
normalize('Riyadh, Riyadh Region, ...')  = بلا تغيير (لا يوجد بديل لـRiyadh) ✅
normalize('Jiddah')                      = 'jeddah'   ✅
normalize('Jeddah')                      = 'jeddah'   ✅ (يبقى كما هو، متوقَّع)
normalize('Medina')                      = 'madinah'  ✅
normalize('Al-Madinah')                  = 'madinah'  ✅
normalize('Al Madinah')                  = 'madinah'  ✅
```
كل الحالات التسع طابقت المتوقَّع تماماً.

### حالة اختبار 114 — إعادة اختبار مكة حياً: الملاحظة لم تعد تظهر خطأً
استدعاء `search_jobs('Backend Developer', 'Makkah, Saudi Arabia')` حي
فعلي (بالمفتاح الجديد، لا محاكاة) بعد الإصلاح:
```
total_count: 14   note present: False
[Indeed] Food Industry Software Developer            | Mecca
[Indeed] Software Engineer (Frontend, Backend, ...)  | Mecca
```
مقارنةً بالجولة السابقة (قبل الإصلاح): نفس بالضبط نتائج Indeed بموقع
"Mecca" كانت موجودة فعلياً بالرد الخام، لكن الملاحظة كانت تظهر رغم ذلك
(علة التهجئة). بعد الإصلاح: **نفس نوع النتائج، الملاحظة اختفت بشكل
صحيح**. أُعيد الاستدعاء الحي مرتين إضافيتين للتأكد من عدم كونها صدفة
واحدة — كلتاهما أكّدتا: أي رد فيه ولو نتيجة Indeed واحدة موقعها يحوي
"Mecca" يُسقِط الملاحظة فوراً كما هو متوقَّع (تفصيل الاستدعاءات
والنتائج الخام محفوظ بـ`makkah_fix_evidence.json`).

**اختبار ترشيح التكرار تحديداً (monkeypatch لعزل المنطق عن تذبذب Indeed
الحي)**: وظيفة وهمية "Staff Backend Engineer (NodeJS/Go)" بنفس
العنوان/الشركة من Jooble (موقع "Saudi Arabia") ومن Indeed (موقع "Mecca"
حرفياً)، بطلب "Makkah, Saudi Arabia":
```
عدد النسخ بالنتيجة النهائية: 1        (متوقَّع 1 — الدمج أزال التكرار) ✅
المصدر المُبقى عليه: Indeed            (متوقَّع Indeed — Mecca تطابق Makkah بعد التوحيد) ✅
موقعه المعروض: Mecca                  (بلا تعديل — التطبيع للمقارنة فقط، لا للعرض) ✅
note present: False                    (متوقَّع False) ✅
```

### حالة اختبار 115 — إعادة التحقق من جدة والرياض: صفر ارتداد
```
Jeddah, Saudi Arabia:  total_count=13  note=غائبة  (Indeed: كل النتائج موقعها "Jeddah" — طابقت مباشرة بلا حاجة توحيد أصلاً)
Riyadh, Saudi Arabia:  total_count=15  note=غائبة  (Indeed+ActiveJobsDB: "Riyadh" — نفس الشيء)
```
كلتا المدينتين تعملان بالضبط كما قبل هذي الجولة — التوحيد الجديد لا
يغيّر أي سلوك لمدن لا تحتاج تطبيعاً (Jeddah/Riyadh تُطابَقان مباشرة بلا
حاجة القاموس)، فلا ارتداد.

### حالة اختبار 116 — إصلاح جانبي حقيقي اكتُشف أثناء التحقق: مهلة Indeed Scraper كانت أقصر من زمن استجابته الفعلي
أثناء التحقق عبر الواجهة الفعلية (Playwright، رفع PDF حقيقي + بحث حي)
لمدينة مكة تحديداً، فشل الاستدعاء **مرتين متتاليتين** بمهلة 20 ثانية
(`Read timed out. (read timeout=20)`) رغم نجاح استدعاءات أخرى بنفس
الاستعلام تقريباً خارج المهلة بقليل — دليل حي على أن Indeed Scraper API
(يكشط صفحات حية فعلياً، لا بيانات مخزَّنة مسبقاً) يحتاج أحياناً وقتاً
يقترب من/يتجاوز 20 ثانية حتى بردود ناجحة. رُفعت المهلة لـ35 ثانية
بـ`tools/rapidapi_client.py` — إصلاح موثوقية صغير مرتبط مباشرة بهذي
الجولة (منع فشل صامت يُخفي بالضبط النتائج المدينة-المحدَّدة التي يفحصها
هذا الإصلاح)، لا توسيعاً غير مبرَّر للنطاق.

### التحقق النهائي
- **وحدة `_normalize_city` مختبَرة بـ9 حالات، كلها صحيحة**. ✅
- **مكة**: الملاحظة الخاطئة اختفت بثلاثة استدعاءات حية متتالية ناجحة
  بعد الإصلاح (كانت تظهر دوماً قبله رغم وجود نتائج Mecca فعلية). ✅
- **ترشيح التكرار**: يفضّل الآن Mecca على Saudi Arabia بشكل صحيح، مؤكَّد
  بـmonkeypatch معزول عن تذبذب الشبكة. ✅
- **جدة والرياض**: صفر ارتداد، سلوك مطابق تماماً لما قبل الجولة. ✅
- **الواجهة الفعلية**: تعمل بلا أخطاء console عبر كل الاختبارات (رفع
  PDF حقيقي + بحث حي)، لقطة `MAKKAH_FIXED.png`. ✅
- `location` المعروضة للمستخدم لم تتغيّر (لا استبدال "Mecca" بـ"Makkah"
  بالعرض) — التوحيد داخلي للمقارنة فقط، مقصود. ✅

### الخلاصة
علة تهجئة النقل الحرفي (Makkah/Mecca) أُصلحت بجدول توحيد صغير مُطبَّق
على طرفي كل مقارنة مدينة (طلب المستخدم ونتائج كل مصدر)، مغطياً الحد
الأدنى المطلوب (Makkah/Mecca، Madinah/Medina، Jeddah/Jiddah) بلا إضافة
مدن إضافية تخمينية لم يظهر دليل حاجتها. تحقّق حي متكرر (لا مرة واحدة)
أثبت اختفاء الملاحظة الخاطئة تحديداً حين تتوفر نتائج Mecca فعلية، مع
الحفاظ الكامل على سلوك جدة والرياض بلا ارتداد. عُثر أثناء التحقق على
علة موثوقية صغيرة منفصلة (مهلة Indeed Scraper القصيرة) وأُصلحت فوراً
لأنها كانت تُخفي بصمت بالضبط النوع من الأدلة الذي يحتاج هذا الإصلاح
إثباته.

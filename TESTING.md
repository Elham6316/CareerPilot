...
**تصحيح مهم على العدّ**: بهذا اكتملت **6 من 7** أدوات مذكورة في
`PROJECT_BRIEF.md`: suggest_job_titles, search_jobs, evaluate_match,
suggest_resume_edits, log_application, get_application_status — بالإضافة
لاستخراج نص PDF (خطوة تأسيسية سابقة للأدوات). **`draft_cover_letter`
(بند 6 في الخطة) لسا موجودة فقط كـ schema في `TOOLS_SCHEMA` بدون تنفيذ
فعلي** — لم يُطلب مني تنفيذها بعد ضمن التسلسل اللي اشتغلت عليه (تم تخطّي
ترقيمها في الطلبات المتتالية). لو النموذج حاول يستدعيها الآن سترجع
`{'error': "أداة 'draft_cover_letter' غير مربوطة بعد بتنفيذها الفعلي."}`
(rule 6 عادي، لا انهيار) بدل تنفيذ فعلي.
